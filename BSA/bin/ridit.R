...
###denoise
data=output_table
chrlist=levels(as.factor(data$CHROM))
loess0<-function(chr){
   df1 =data[data$CHROM == chr,]
   if(nrow(df1) < 100){
        return=data.frame(chr=df1$CHROM,pos=df1$POS,loess=1)
   }else{
        result=loess(fdr ~ POS,data=df1) %>% predict(df1$POS)
        return=data.frame(chr=df1$CHROM,pos=df1$POS,loess=result)
   }
   return
}
