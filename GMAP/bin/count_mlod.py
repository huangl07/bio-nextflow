...
def abstract_input(chunk):
    chr1 = chunk[0].str.split('-', 1,expand=True)[0]
    chr2 = chunk[1].str.split('-', 1,expand=True)[0]
...
def max_MLOD_group(df, outfile1):
    summarize_df = df.groupby(["Loci_1", "Loci_2"])['MLOD'].mean().sort_values(ascending=False)
    chr_interval = df.groupby(["Loci_1", "Loci_2"])['MLOD'].mean().sort_values(ascending=False).index.to_list()[0]
    max_MLOD_group_result = chr_interval + (str(round(df.groupby(["Loci_1", "Loci_2"])['MLOD'].mean().sort_values(ascending=False).to_list()[0], 2)),)
