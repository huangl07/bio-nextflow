...
print OUT "Chromosome ID\tStart\tEnd\tSNP number\tInDel number\tEffsnp\tEffInDel\n";
...
print OUT "Chromosome ID\tStart\tEnd\tGene Number\tGene with Eff variation\n";
foreach my $chr (sort keys %region){
    foreach my $region(sort keys %{$region{$chr}}){
